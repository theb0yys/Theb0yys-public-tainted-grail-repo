...
foreach ($root in $indexRoots) {
    ...
    foreach ($readme in $readmes) {
        ...
        foreach ($sibling in $siblings) {
            ...
            if (-not $linked) {
                $failures.Add("Unindexed sibling Markdown page: $dirRel/$($sibling.Name) is not linked from $readmeRel")
            }
        }
    }
}
...
foreach ($root in @('mechanics/','investigate/')) {
    ...
    foreach ($metadataPath in $rootFiles) {
        ...
        if ($dateMatch.Success) {
            $dateValue = $dateMatch.Groups['date'].Value.Trim()
...
foreach ($path in $markdownFiles) {
    ...
    if ($path.StartsWith('contributing/authoring/reference-audit/packets/')) {
        continue
    }
    ...
    foreach ($paragraph in $paragraphs) {
        ...
        if ($normalized.StartsWith([string][char]96)) {
            continue
        }
