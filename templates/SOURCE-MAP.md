...
Private production implementation is not copied wholesale. The named starters extract portable feature rules such as guards, scaling, bounded state, readiness gates, ownership decisions and sampling logic, while leaving exact game hooks/runtime adapters for the author to add from verified evidence.
