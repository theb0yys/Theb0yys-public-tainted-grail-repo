# How-to Guides

Choose the task you want to complete. Use [Knowledge](../../knowledge/README.md) when you need technical background, [Research](../../research/README.md) when behaviour is still unknown, and [Troubleshooting](../troubleshooting/README.md) when something has failed.

## Content

- [Items](items/README.md)
- [Weapons](weapons/README.md)
- [Armour integration](armour/native-integration.md)
- [Creatures](creatures/README.md)

## Runtime and tooling

- [Gameplay changes](gameplay/README.md)
- [Debugging and diagnostics](diagnostics/README.md)
- [Validate compatibility](compatibility/validation.md)
- [Reverse engineering and discovery](reverse-engineering/discovery.md)
- [Interoperability](interoperability/README.md)
- [UI](ui/README.md)
- [Audio](audio/README.md)
- [Rendering](rendering/README.md)
- [World](world/README.md)
- [Saving and persistence](saving/README.md)
