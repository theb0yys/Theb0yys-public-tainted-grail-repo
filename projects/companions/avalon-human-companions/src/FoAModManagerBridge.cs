...
internal static class FoAModManagerBridge
{
    ...
    internal static void SetCustomUiScope(string ownerId, bool active, bool freezeWorld)
    {
        ...
        catch (Exception ex)
        {
            if (!s_loggedInvokeFailure)
            {
                ...
                Debug.LogWarning($"Avalon Human Companions could not update FoA Mod Manager shared UI scope: {ex.GetType().Name}: {ex.Message}");
            }
    ...
    internal static void SetControllerCursorScope(string ownerId, bool active)
    {
        ...
        catch (Exception ex)
        {
            ...
            {
                s_loggedInvokeFailure = true;
                Debug.LogWarning($"Avalon Human Companions could not update FoA Mod Manager controller cursor scope: {ex.GetType().Name}: {ex.Message}");
            }
        }
    }
