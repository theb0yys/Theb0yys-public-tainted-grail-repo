...
- Requested behavior: split bear companion work into its own diagnostic/scaffold-only mod before any advanced behavior.
- Files expected to change: `mods/avalon-bear-companion/**`.
- Risk level: Low for scaffold; high for future companion behavior.

## Target

- Assembly: none beyond BepInEx and Unity input references.
- Class: `AvalonBearCompanion.Plugin`.
- Runtime object: no game runtime object is modified.
- Evidence: `Spec_AnimalBear` has FOA-Diagnostic Tool route-context spawner evidence and a review row at `AVALON-PET-BEAR-001`, but is only a predator companion review candidate. It is not approved for companion behavior.
